...
int main(int argc, char** argv) {
	...
	//Histogram drawing sizes
	int hist_w = 512; int hist_h = 400;
	int bin_w = cvRound( (double) hist_w/histSize );

	/// Set the ranges ( for N, S, E, W)
	float range[] = { 0, 5000 } ;
		...
		if (func == 0 || func == 1) {
			...
			if (func == 0) {
				...
				if (traj) {
					for (unsigned int i = 0; i < nextPts.size(); i++) {
						if (status[i]) {
							...
							//Calculate angle and norm of vectors
							double angle = atan((y2-y1)/(x2-x1));	//1º Quadrante

							if((y2-y1)>=0 && (x2-x1)<0){						//2º Quadrante
								angle = angle + CV_PI;
							}
							else if((y2-y1)<0 && (x2-x1)<0){			//3º Quadrante
								angle = angle + CV_PI;
							}
							else if((y2-y1)<0 && (x2-x1)>0){			//4º Quadrante
								angle = angle + CV_PI*3/2;
							}
							else if((y2-y1)>0 && (x2-x1)==0){			//Norte
								angle = CV_PI/2;
							}
							else if((y2-y1)<0 && (x2-x1)==0){			//Sul
								angle = angle + 2*CV_PI;
							}

							double mod = norm(nextPts, prevPts);

							if((angle >= -CV_PI/4) && (angle < CV_PI/4)){
								vechist[0] = vechist[0] + mod;		//Increase counter of EAST movement
							}

							else if((angle >= CV_PI/4) && (angle < CV_PI*3/4)){
								vechist[1] = vechist[1] + mod;		//Increase counter of NORTH movement
							}

							else if((angle >= CV_PI*3/4) && (angle < CV_PI*5/4)){
								vechist[2] = vechist[2] + mod;		//Increase counter of WEST movement
							}

							if((angle >= CV_PI*5/4) && (angle < CV_PI*7/4)){
								vechist[3] = vechist[3] + mod;		//Increase counter of SOUTH movement
							}
						}
					}
				}
				...
				for( int i = 1; i < histSize; i++ ) {
					line( histImage, Point( bin_w*(i-1), hist_h - cvRound(n_hist.at<float>(i-1)) ) ,
													 Point( bin_w*(i), hist_h - cvRound(n_hist.at<float>(i)) ),
													 Scalar( 255, 0, 0), 2, 8, 0  );
					line( histImage, Point( bin_w*(i-1), hist_h - cvRound(s_hist.at<float>(i-1)) ) ,
													 Point( bin_w*(i), hist_h - cvRound(s_hist.at<float>(i)) ),
													 Scalar( 0, 255, 0), 2, 8, 0  );
					line( histImage, Point( bin_w*(i-1), hist_h - cvRound(e_hist.at<float>(i-1)) ) ,
													 Point( bin_w*(i), hist_h - cvRound(e_hist.at<float>(i)) ),
